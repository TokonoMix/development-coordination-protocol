...
PCP's hardest design constraint is staying *purely semantic*. This document
...
concern. The test applied throughout: **does PCP define behaviour that acts on
...
|---|---|---|
| `message_id` | delivery/idempotency id | Audit/correlation identity only. PCP does no dedup/delivery; the spec forbids reading it as a delivery id. |
...
| `correlation_id` | a transport thread id | A semantic link to a prior PCP message; no delivery meaning, no authenticity guarantee. |
...
| `priority` | a scheduler input | A descriptive label; PCP defines no scheduler and no behaviour over it. |
| `dependency_type: blocks` | a workflow ordering instruction | States a relationship; PCP enforces, orders, or schedules nothing on it. |
| `milestone.target_date` | a deadline / scheduling directive | Descriptive intent; PCP derives no deadline state and performs no scheduling. |
| status enums | a state machine | A vocabulary of states; PCP enforces no transitions. |
| `ReviewRequest.status: approved` | an authorization to proceed | A record that an approval occurred; PCP gates nothing. A separate system decides what to do. |
...
If a change would make PCP *do* something — route, authenticate, authorize,
schedule, enforce a transition, orchestrate, or execute — it is out of scope by
definition, no matter how convenient. PCP only records that something changed.
